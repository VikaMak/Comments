		...
		if (isset($_POST['login']) ) {
			...
			if ( empty($this->errors['login']) and empty($this->errors['password']) and empty($this->errors['email']) ) {
				...
				if ( empty($this->errors['login']) and empty($this->errors['password']) and empty($this->errors['email']) ) {
					...
					$this->message = 
					'Пройдите по ссылке для активации вашего аккаунта: http://viktoryiam.000webhostapp.com/public/home/activation?id='.$this->id.'&hash='.$this->hash;
					...
					header('Location: /public/home/#popup2');
					die();
				}
